...
## Modified to remove the RNAfold dependencies
def log_status(gtf_fn, ref_fn, prefix, tpm_fn, out_dir):
    """
    Logging the status
...
def module_merge(prefix, tpm_fn, out_dir):
    ...
    # execute
    dat = MergeDF(bed, tpm_fn, out_dir)
    ## Modified to remove the RNAfold dependencies
...
## Modified to remove the RNAfold dependencies
def scikit_ribo_build(gtf_fn, ref_fn, prefix, tpm_fn, out):
    """
    ...
    ## Modified to remove the RNAfold dependencies
    log_status(gtf_fn, ref_fn, prefix, tpm_fn, out)
...
if __name__ == '__main__':
    ...
    ## Modified to remove the RNAfold dependencies
    if args.g != None and args.f != None and args.p != None and args.t != None and args.o != None:
        ...
        tpm = args.t
        output = args.o
        scikit_ribo_build(gtf, fasta, pre, tpm, output)
    else:
