...
class VisualizeAsite(object):
    ...
    def plot(self):
        ...
        # normed=True was removed to avoid an error with map
        g0.map(plt.hist, "asite", color="steelblue", bins=bins, lw=0)
        if not self.RelE:
        ...
        g1 = sns.FacetGrid(self.training, row="3_offset", col="read_length", margin_titles=True,
                           col_order=list(range(10, 36)), row_order=list(range(3)))
        # normed=True was removed to avoid an error with map
        g1.map(plt.hist, "asite", color="steelblue", bins=bins, lw=0)
        if not self.RelE:
...
class PredictAsite(object):
    ...
    """
    def __init__(self, training=None, cds=None, classifier="rf", RelE=None, pre=None, output=None, directory=None, site=None):
        self.training = training
        self.cds = cds
    ...
    def recoverAsite(self):
        ## Edited code to allow A, P and E site using inpit from user
        # adjust shift in cds data according to choice
        if self.site == "A":
            shift = 0
        elif self.site == "P":
            shift = 3
