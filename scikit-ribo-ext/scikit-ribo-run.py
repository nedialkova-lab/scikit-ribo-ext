...
def aln_module(bam, mapQual, minReadLen, maxReadLen, rele, out, start, cds, posIdx, nt, geneSubset, directory, prefix):
    """
    ...
    aln = BamProcess(bam, mapQual, minReadLen, maxReadLen, rele, out, start, cds, posIdx, nt, geneSubset, directory, prefix)
...
def asite_module(trainingData, cdsData, rele, prefix, out, directory, site):
    """
    ...
    classifier = "rf"
    model = PredictAsite(trainingData, cdsData, classifier, rele, prefix, out, directory, site)
...
def scikit_ribo(bam, directory, prefix, mapQual, minReadLen, maxReadLen, rele, out, unmap, cv, site, geneSubset):
    """
    ...
    :param site: str, 'A', 'P', or 'E'
    :return: None
    """
    # log status
    start, cds, posIdx, nt, lambda_min = log_status(bam, directory, prefix, out, cv)
    # load data
    trainingData, cdsData = aln_module(bam, mapQual, minReadLen, maxReadLen, rele, out, start, cds, posIdx, nt, geneSubset, directory, prefix)
    # predict a-site
    dataFrame = asite_module(trainingData, cdsData, rele, prefix, out, directory, site)
    # model te
...
if __name__ == '__main__':
    ...
    parser.add_argument("--site", help = "Ribosome A- P- or E-site for which to generate codon dwell times", default = "A", choices=['A', 'P', 'E'])
    parser.add_argument("--genesubset", help = "Subset of transcripts to use for training and prediction. Results returned will be for this subset \
        only. Input is a text file, one transcript per line. Please ensure the IDs used match those in the GTF supplied when making the scikit-ribo index \
        (usually Ensembl IDs).")
    ...
    if args.i != None and args.f != None and args.p != None and args.o != None:
        ...
        scikit_ribo(input_bam, folder, pre, mapq, minRL, maxRL, RelE, output, unmap_fn, cv, site, geneSubset)
    else:
