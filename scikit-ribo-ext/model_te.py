...
class ModelTE(object):
    ...
    def glmnetArr(self):
        ...
        ## Modified to remove the RNAfold dependencies
        varsNamesLst = cateVarsNames
        # prepare input arrays
        cateVars = dataprocess().sparseDf(cateVars)
        ## Modified to remove the RNAfold dependencies
        #downstreamSL = np.array(self.df["downstreamSL"].as_matrix().reshape(int(len(self.df["downstreamSL"])), 1), dtype=np.float64)
        sparseX = sparse.hstack([cateVars], format="csc", dtype=np.float64)
    ...
    ##    ## Modified: this function is not called elsewhere
    ##    def nbGlm(self):
    ##
    ##        """
    ##
